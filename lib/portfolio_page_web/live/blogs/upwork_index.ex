defmodule PortfolioPageWeb.BlogLive.UpworkIndex do
  ...
  def render(assigns) do
    ~H"""
    <Layouts.app type="upwork" flash={@flash}>
          ...
          <section :if={@live_action == :index} class="flex-2 lg:flex-3">
            ...
            >
              <li :for={{dom_id, post} <- @streams.posts} id={dom_id}>
                <.blog_post_link type="upwork" post={post} />
              </li>
            </ul>
          </section>

          <section :if={@live_action == :show} class="flex-2 lg:flex-3">
            <.blog_section type="upwork" post={@post} />
          </section>

          <section class="flex-1 flex flex-col gap-4">
            <!-- Search Bar -->
            <.blog_sidebar type="upwork"
        ...
        <nav aria-label="Post tags">
          <ul class="flex flex-row gap-2 flex-wrap">
            <li :for={tag <- @post.tags}>
              <.link class="btn btn-secondary btn-sm" patch={~p"/upwork/blogs?tag=#{tag}"}>{tag}</.link>
            </li>
          </ul>
        </nav>
  ...
  def handle_event("filter_tags", %{"value" => value}, socket) do
    {:noreply, socket |> push_patch(to: "/upwork/blogs?tag=#{value}")}
  end

  def handle_event("search", %{"value" => value}, socket) do
    {:noreply, socket |> push_patch(to: "/upwork/blogs?search=#{value}")}
  end
